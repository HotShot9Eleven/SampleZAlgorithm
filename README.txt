...
and X (desired dollar amount). The application should take both inputs and leverage the Zappos API 
(http://developer.zappos.com/docs/api-documentation) to create a list of Zappos products whose combined 
values match as closely as possible to X dollars. For example, if a user entered 3 (# of products) and $150, 
the application would print combinations of 3 product items whose total value is closest to $150.
...
The following recursive solution will create a list of possible list of price combination that statisfy the
above two condition.
...
If the input parameters are N = 3 and C = 10, and since we want an approximation the above algorithm will
also print price combinations that are + or -  the total cost. If + or - 2 is allowed the recursive function 
...
Catesian product is found for the above lists and using the hashamp to generate all possible item combinations
